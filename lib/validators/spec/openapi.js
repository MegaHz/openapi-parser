...
function validatePathParameters(params, pathId, operationId) {
  ...
  // MegaHz Fix - 2022-11-29
  // Not the prettiest change, but targets and fixes the malformed OS endpoints

  pathId = pathId.replace('/chain}/', '/{chain}/').replace('{token_id}/validate', '{token_id}/{validate}');
  ...
  // MegaHz Fix - 2022-11-29
  // {owners} parameter is causing problems, but I am not using it. So skipping that error

  // if (placeholders.length > 0) {
  //   throw ono.syntax(`Validation failed. ${operationId} is missing path parameter(s) for ${placeholders}`);
  // }

  if (placeholders.length > 0 && placeholders[0] !== '{owners}') {
    throw ono.syntax(`Validation failed. ${operationId} is missing path parameter(s) for ${placeholders}`);
  }
}
